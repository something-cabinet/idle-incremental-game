...
import { dismissStory, isFirstRunArrival, storyBeatDef } from '../game/story';
 ...
 *  The a1-arrival beat is suppressed here because the landing threshold owns it. */
export function StoryModal() {
  const store = useGameStore();
  const state = useGameState();
  const battleOpen = useBattleOpen();

  // The landing threshold owns a1-arrival for first-run saves
  const suppressArrival = isFirstRunArrival(state);
  const pending = state.pendingStories.filter(
    (id) => !(suppressArrival && id === 'a1-arrival'),
  );
  const beatId = battleOpen ? undefined : (pending[0] as string | undefined);
